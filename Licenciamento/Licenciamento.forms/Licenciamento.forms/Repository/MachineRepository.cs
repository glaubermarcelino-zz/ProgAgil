...
namespace Licenciamento.forms.Repository
{
    public class MachineRepository : IMachine
    {
        public IMachine service { get; set; }
        public async Task<Machine> GetMachineId(string id)
        {
            var machine = await service.GetMachineId(id);
            if (machine != null) return machine;
            return null;
        }
        public MachineRepository()
        {
            //service = RestService.For<IMachine>("http://localhost:5000/api/v1");
            service = RestService.For<IMachine>("http://programeja.com/api/v1/");
        }

        public string GetMachineIdLocal()
        {
        ...
        public async Task<Machine> AutenticateMachine(string MachineId)
        {
            //Não tem erro não é so tratamento mesmo
            try
            {
                Machine retorno = await service.AutenticateMachine(MachineId);
                return retorno;
            }
        ...
        public Task<Machine> GetMachinePermission(string Id)
        {
            throw new NotImplementedException();
        }

        public async Task<Machine> RegisterMachine(Machine machine)
        {
            if (machine != null)
            {
                 return await service.RegisterMachine(machine);
            }
            return null;
        }

        public async Task<Machine> UpdateMachine(Machine machine, string machineId)
        {
            if (machine.chave_mach == machineId)
            {
                return await service.UpdateMachine(machine,machineId);
            }
            return null;
        }
    }
